...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = i; j < matrix.length; j += 1) {
      [rotate[i][j], rotate[j][i]] = [rotate[j][i], rotate[i][j]];
    }
  }
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix.length / 2; j += 1) {
      [rotate[i][j], rotate[i][matrix.length - 1 - j]] = [
        rotate[i][matrix.length - 1 - j],
        rotate[i][j],
      ];
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    let newStr = '';
    let oddChars = '';
    for (let j = 0; j < shufflStr.length; j += 1) {
      if (j % 2 === 0) {
        newStr += shufflStr[j];
      } else {
        oddChars += shufflStr[j];
      }
    }
    newStr += oddChars;
    shufflStr = newStr;
